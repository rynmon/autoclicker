fn main() {
    ...
    if target_os == "windows" {
        // Use conditional compilation to only compile this on Windows
        #[cfg(windows)]
        {
            ...
            if let Ok(mut ico_file) = std::fs::File::create(ico_path) {
                if let Err(e) = img.write_to(&mut ico_file, ImageFormat::Ico) {
                    eprintln!("Warning: Could not write icon file: {}", e);
                } else {
        ...
        #[cfg(not(windows))]
        {
            // Skip icon generation when cross-compiling from non-Windows
        }
    }
    // On non-Windows targets, do nothing
}
